...
CLUES_FILE_PATHS = [
    # os.path.join(os.path.dirname(__file__), "etc/apps.json"),
    "/etc/wad/apps.json",
    # "/etc/wad/old.json"
]
...
class _Clues(object):
    ...
    def read_clues_from_file(filename):
        ...
        try:
            ...
            logging.error(
                "Error while opening clues file, terminating: %s", tools.error_to_str(e)
            )
            raise
        ...
        try:
            ...
            logging.error(
                "Error while reading JSON file, terminating: %s", tools.error_to_str(e)
            )
            raise

        json_data.close()
        categories = dict((k, v["name"]) for k, v in six.iteritems(clues["categories"]))
        apps = clues["technologies"]
    ...
    def add_categories_str(self):
        for app in self.apps:
            self.apps[app]["catsStr"] = six.moves.reduce(
                lambda a, b: "%s,%s" % (a, b),
        ...
        for app in self.apps:
            ...
            for key in list(self.apps[app]):
                if key in ["script", "html", "url"]:
                    try:
                        regexps[key + "_re"] = list(
                            six.moves.map(self.compile_clue, self.apps[app][key])
                        )
                    except sre_constants.error:
                        del self.apps[app][key]
                if key in ["meta", "headers", "js", "cookies"]:
                    try:
                        # regexps[key + "_re"] = dict(
                        #     (entry, self.compile_clue(self.apps[app][key][entry]))
                        #     for entry in self.apps[app][key]
                        # )

                        regexps[key + "_re"] = dict()
                        for entry in self.apps[app][key]:
                            if type(self.apps[app][key][entry]) is list:
                                # This weird hack for Abicart's list of generators
                                for item in self.apps[app][key][entry]:
                                    regexps[key + "_re"][item] = self.compile_clue(item)
                            else:
                                regexps[key + "_re"][entry] = self.compile_clue(
                                    self.apps[app][key][entry]
                                )
                    except sre_constants.error:
                        del self.apps[app][key]
